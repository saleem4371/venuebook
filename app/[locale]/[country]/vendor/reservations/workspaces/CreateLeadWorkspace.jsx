...
import {
  UserPlus, Calendar, MapPin, Phone, Mail, Users, AlignLeft,
  X, AlertTriangle, Building2, Clock, Zap, CheckCircle2,
  Banknote, Tag, Save, ChevronDown, ChevronUp, TrendingUp,
} from "lucide-react";
...
/* ── Priority config ─────────────────────────────────────────── */
const PRIORITY_CFG = {
  low:    { dot: "bg-blue-400",   active: "bg-blue-50 dark:bg-blue-950/30 border-blue-300 dark:border-blue-700/50 text-blue-700 dark:text-blue-300"   },
  medium: { dot: "bg-amber-400",  active: "bg-amber-50 dark:bg-amber-950/30 border-amber-300 dark:border-amber-700/50 text-amber-700 dark:text-amber-300" },
  high:   { dot: "bg-orange-500", active: "bg-orange-50 dark:bg-orange-950/30 border-orange-300 dark:border-orange-700/50 text-orange-700 dark:text-orange-300" },
  urgent: { dot: "bg-rose-500",   active: "bg-rose-50 dark:bg-rose-950/30 border-rose-300 dark:border-rose-700/50 text-rose-700 dark:text-rose-300"   },
};

/* ── Tags config ────────────────────────────────────────────── */
const TAGS_CFG = [
  { key: "vip",     tKey: "tagVIP",     active: "bg-violet-50 dark:bg-violet-950/30 border-violet-300 dark:border-violet-700/50 text-violet-700 dark:text-violet-300" },
  { key: "repeat",  tKey: "tagRepeat",  active: "bg-blue-50 dark:bg-blue-950/30 border-blue-300 dark:border-blue-700/50 text-blue-700 dark:text-blue-300"             },
  { key: "urgent",  tKey: "tagUrgent",  active: "bg-rose-50 dark:bg-rose-950/30 border-rose-300 dark:border-rose-700/50 text-rose-700 dark:text-rose-300"             },
  { key: "premium", tKey: "tagPremium", active: "bg-amber-50 dark:bg-amber-950/30 border-amber-300 dark:border-amber-700/50 text-amber-700 dark:text-amber-300"       },
];

/* ── Default form state ─────────────────────────────────────── */
const EMPTY = {
  venue: "", eventDate: "", shift: "",
  name: "", phone: "", email: "",
  source: "", referredBy: "", guestCount: "", customerType: "new",
  budget: "", eventType: "", priority: "medium",
  address: "", notes: "",
  tags: [],
};
        ...
        <div className="flex items-center gap-2 mb-5">
          {Icon && (
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-50 dark:bg-violet-950/40 shrink-0">
              <Icon size={13} className="text-violet-600 dark:text-violet-400" aria-hidden="true" />
            </span>
          )}
  ...
  return (
    ...
    >
      <Calendar
        ...
        aria-hidden="true"
      />
      <span className={`text-sm flex-1 ${value ? "text-gray-900 dark:text-gray-100 font-medium" : "text-gray-400 dark:text-gray-500"}`}>
        {formatted ?? label}
      </span>
      {value && (
        <button
    ...
    <div className="flex flex-wrap gap-2">
      {Object.entries(PRIORITY_CFG).map(([key, cfg]) => {
        ...
        return (
          ...
          >
            <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${isActive ? cfg.dot : "bg-gray-300 dark:bg-gray-600"}`} />
            {t(`addLeadModal.priority${key.charAt(0).toUpperCase() + key.slice(1)}`)}
          </button>
        );
      })}
    </div>
    ...
    <div className="flex flex-wrap gap-2">
      {TAGS_CFG.map(({ key, tKey, active }) => {
        ...
        return (
          ...
          >
            {on && <CheckCircle2 size={11} className="shrink-0" aria-hidden="true" />}
            {t(`addLeadModal.${tKey}`)}
          </button>
        );
      })}
...
/* ── Right summary panel ─────────────────────────────────────── */
function SummaryPanel({ venue, t }) {
  const venueBookings = useMemo(
    () => (venue ? MOCK.filter((i) => i.venue === venue) : []),
    [venue],
  );
  ...
  const stats = useMemo(() => {
    ...
    const confirmed = venueBookings.filter((i) => i.workflowState === "CONFIRMED").length;
    const avgStr    = avg >= 100000 ? `₹${(avg / 100000).toFixed(1)}L` : `₹${(avg / 1000).toFixed(0)}K`;
    return { total, confirmed, avgStr };
  }, [venueBookings]);

  const recent = useMemo(
    () => (venue ? venueBookings : MOCK).slice(0, 4),
    [venue, venueBookings],
  );
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        <div className="p-4">
          <div className="flex items-center gap-2 mb-4">
            <Building2 size={13} className="text-violet-500 dark:text-violet-400 shrink-0" />
          ...
          {venue ? (
            <>
              <p className="text-sm font-bold text-gray-900 dark:text-gray-50 mb-3 truncate">{venue}</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: t("addLeadModal.statBookings"),  value: stats?.total ?? 0   },
                  { label: t("addLeadModal.statConfirmed"), value: stats?.confirmed ?? 0 },
                  { label: t("addLeadModal.statAvg"),       value: stats?.avgStr ?? "—"  },
                ].map(({ label, value }) => (
          ...
          {recent.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-2.5 py-2.5">
                ...
                <span className={`shrink-0 inline-flex items-center text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${cfg.color}`}>
                  {cfg.label}
                </span>
              </div>
            );
      ...
      <div className="bg-violet-50 dark:bg-violet-950/20 border border-violet-100 dark:border-violet-900/30 rounded-2xl p-4">
        <div className="flex items-start gap-2">
          <Zap size={12} className="text-violet-500 dark:text-violet-400 mt-0.5 shrink-0" aria-hidden="true" />
          <div>
            ...
            <ul className="space-y-1">
              {["panelTip1", "panelTip2", "panelTip3"].map((k) => (
                <li key={k} className="text-[11px] text-violet-600/80 dark:text-violet-400/80 flex items-start gap-1">
...
export default function CreateLeadWorkspace() {
  ...
  const checkDuplicate = (field, value) => {
    if (!value || value.replace(/\D/g, "").length < 7) { setDuplicate(null); return; }
    const found = MOCK.find((i) =>
      field === "phone"
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-2 sm:self-start">
            {form.priority && (
              <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full border ${PRIORITY_CFG[form.priority]?.active}`}>
                <span className={`h-1.5 w-1.5 rounded-full ${PRIORITY_CFG[form.priority]?.dot}`} />
                {t(`addLeadModal.priority${form.priority.charAt(0).toUpperCase() + form.priority.slice(1)}`)}
              </span>
            )}
            {form.source && (
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <SectionCard title={t("addLeadModal.sectionEvent")} icon={Calendar}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                {/* Event Date */}
                <div>
                  <label className={labelCls}>{t("addLeadModal.eventDate")}</label>
                  <DateInput
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Morning">{t("addLeadModal.shiftMorning")}</option>
                    <option value="Afternoon">{t("addLeadModal.shiftAfternoon")}</option>
                    <option value="Evening">{t("addLeadModal.shiftEvening")}</option>
                  </select>
                </div>
            ...
            <SectionCard title={t("addLeadModal.sectionLeadInfo")} icon={TrendingUp}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">

                {/* Lead Source */}
                <div>
                  <label className={labelCls}>{t("addLeadModal.leadSource")}</label>
                  <select
                    value={form.source}
                  ...
                  >
                    ...
                    {[
                      ["sourceWalkIn","Walk-in"], ["sourcePhone","Phone Call"],
                      ["sourceWhatsApp","WhatsApp"], ["sourceInstagram","Instagram"],
                      ["sourceFacebook","Facebook"], ["sourceGoogle","Google"],
                      ["sourceEmail","Email"], ["sourceReferral","Referral"],
                      ["sourceOther","Other"],
                    ].map(([key, val]) => (
                      <option key={val} value={val}>{t(`addLeadModal.${key}`)}</option>
                    ))}
                  </select>
                </div>
                ...
                {/* Referred By */}
                <div>
                  <label className={labelCls}>{t("addLeadModal.referredBy")}</label>
                  <input
                    type="text"
              ...
              <div className="mt-4">
                <label className={labelCls}>{t("addLeadModal.customerType")}</label>
                <div className="flex flex-wrap gap-2">
                  {[
            ...
            <SectionCard title={t("addLeadModal.sectionBooking")} icon={Banknote}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm font-medium text-gray-400 dark:text-gray-500 pointer-events-none">₹</span>
                    <input
                      type="text"
                      value={form.budget}
                ...
                {/* Event Type */}
                <div>
                  <label className={labelCls}>{t("addLeadModal.eventType")}</label>
                  <select
                ...
                <div>
                  <label className={labelCls}>{t("addLeadModal.priority")}</label>
                  <PrioritySelector value={form.priority} onChange={(v) => set("priority", v)} t={t} />
                </div>
              ...
              <div>
                ...
                <TagsSelector value={form.tags} onChange={(v) => set("tags", v)} t={t} />
              </div>
            </SectionCard>
            ...
            {/* ── Section 6: Notes ──────────────────────── */}
            <SectionCard title={t("addLeadModal.sectionNotes")} icon={AlignLeft}>
              <textarea
                value={form.notes}
            ...
            <AnimatePresence>
              {panelOpen && (
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <SummaryPanel venue={form.venue} t={t} />
                </motion.div>
              )}
            </AnimatePresence>
