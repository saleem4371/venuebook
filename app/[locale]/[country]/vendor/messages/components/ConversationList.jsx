 ...
 * ConversationList
 * ──────────────────────────────────────────────────────────────────
 * Left sidebar panel for the Messages page. Structure/view mirrors the
 * customer-facing ConversationList (app/[locale]/[country]/messages/
 * components/ConversationList.jsx) — same header layout (back button +
 * title + unread badge), same card anatomy, same logical/RTL-safe
 * properties, same glass scroll-arrow tabs. Category taxonomy stays
 * the vendor's own (Guests/Leads/Bookings/Team/Support/System) via
 * `../_data`, and all copy is resolved through next-intl so nothing
 * is hardcoded.
 *
 * Features:
...
export default function ConversationList({
  ...
  const [category, setCategory] = useState(
    initialCategory && MOCK_CATEGORIES.some((c) => c.key === initialCategory)
      ? initialCategory
      : "all"
  );

  useEffect(() => {
    if (initialCategory && MOCK_CATEGORIES.some((c) => c.key === initialCategory)) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white dark:bg-gray-950">
      ...
      {/* ── Header ──────────────────────────────────────────── */}
      <div className="relative z-10 px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800 shrink-0 bg-white dark:bg-gray-950">
      ...
      {/* ── Conversation list ────────────────────────────────── */}
      <div className="flex-1 min-h-0 overflow-y-auto md:pb-0 pb-[76px] [scrollbar-width:thin] [scrollbar-color:theme(colors.gray.200)_transparent] dark:[scrollbar-color:theme(colors.gray.800)_transparent]">
        {filtered.length === 0 ? (
