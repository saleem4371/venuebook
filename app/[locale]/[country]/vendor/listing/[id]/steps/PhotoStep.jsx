...
// Used only to detect whether the cover photo actually changed.
function getPhotoKey(photo) {
  if (!photo) return null;
  if (typeof photo === "string") return photo;
  return photo.id || photo.image_id || getPhotoUrl(photo);
}
...
export default function PhotoStep({
  ...
  onAddImageFile = async () => {}, // called for each newly added photo so it can be uploaded to the backend
  onUpdateCoverImage = async () => {}, // called ONLY when the cover photo (index 0) actually changes
}) {
  ...
  // Keep a ref mirror of photos so handleDrop always reads the latest
  // array even if something async/batched happens between dragstart and drop.
  ...
  const handleAddFiles = (files) => {
    ...
    // These are genuinely new photos — always sync to backend.
    newItems.forEach((item) => onAddImageFile(item));

    // If the gallery was empty, the first new item is now the cover —
    // fire the dedicated cover-update call too.
    if (wasEmpty && newItems.length) {
      onUpdateCoverImage(newItems[0]);
    }
  };
  ...
  async function deleteImageFromServer(image) {
    try {
      ...
      const imageUrl =
        typeof image === "string"
          ? image
          : image.image || image.images || image.url || image.path || null;
  ...
  // IMPORTANT FIX: native HTML5 drag & drop requires dataTransfer.setData()
  // to be called inside onDragStart. Without it, some browsers (Firefox
  // especially, some embedded webviews too) will refuse to fire drop
  // events at all, so the whole reorder — and therefore the cover-update
  // call — silently never happens.
  const handleDragStart = (e, idx) => {
    ...
    if (e?.dataTransfer) {
      ...
      // Some browsers require actual data to be set for drag to work.
      try {
        e.dataTransfer.setData("text/plain", String(idx));
      } catch (err) {
        // no-op — some browsers throw in odd sandboxed contexts
      }
    }
  };
  ...
  const handleDrop = async (e, toIdx) => {
    ...
    // Prefer dataTransfer (most reliable), fall back to state.
    let fromIdx = dragIdx;
    if (e?.dataTransfer) {
    ...
    const oldCoverKey = getPhotoKey(currentPhotos[0]);
    const newCoverKey = getPhotoKey(normalized[0]);
    const coverChanged = oldCoverKey !== newCoverKey;
    ...
    // Reordering alone never syncs to the backend — only a genuine cover
    // change does, and it goes through its own dedicated call.
    if (coverChanged) {
      console.log("Cover photo changed — syncing to backend:", normalized[0]);
      try {
        await onUpdateCoverImage(normalized[0], listingId);
      } catch (err) {
        console.error("onUpdateCoverImage failed:", err);
      }
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (type === "main-photo") {
      ...
      // If the photo that was removed was the cover, the new index-0 photo
      // is now the cover — sync that separately, same as the reorder path.
    ...
    if (type === "section-image") {
      ...
      if (sec) {
        ...
        updateSection(sectionId, {
          images: sec.images.filter((_, i) => i !== imgIdx),
        });
    ...
    if (type === "section-delete") {
      ...
      setForm((p) => ({
        ...p,
        photoSections: p.photoSections.filter((s) => s.id !== sectionId),
      }));

      if (sec?.images?.length) {
        await Promise.all(sec.images.map((img) => deleteImageFromServer(img)));
      }
    }
    ...
    <div className="space-y-6">
      ...
      {photos.length > 0 ? (
        ...
        >
          ...
          >
            <Upload size={26} style={{ color: tk.dimmed }} />
          </div>

          <div className="text-center">
            <p className="text-[14px] font-semibold" style={{ color: tk.text }}>
              Drop photos here or{" "}
              <span style={{ color: theme.accent }}>browse</span>
            </p>
      ...
      >
        ...
        >
          ...
          {tipsOpen ? (
            <ChevronUp size={14} style={{ color: tk.dimmed }} />
          ) : (
            <ChevronDown size={14} style={{ color: tk.dimmed }} />
          )}
        </button>
      ...
      {/* PHOTO SECTIONS */}
      <div className="pt-2">
        <div className="h-px mb-6" style={{ background: tk.border }} />
...
function PhotoSectionsPanel({
  ...
  /* Description was never required, just an unused extra field on the
     creation form — dropped so adding a section is a single input. */
  const handleAdd = () => {
  ...
  const categoryNames = Object.values(categorys?.category || {}).flat();

  const merged = [
    ...categoryNames,
    ...(Array.isArray(availableSuggestions) ? availableSuggestions : []),
  ].filter(Boolean);
          ...
          <SectionCard
            ...
            onRemove={() =>
              openConfirm({
                ...
                message: "Delete this section and all images?",
              })
            }
            onAddImages={(files) => onAddImages(section.id, files)}
            onRemoveImage={(idx) =>
              openConfirm({
                type: "section-image",
                sectionId: section.id,
...
function SectionCard({
  ...
  const atMax = imageCount >= MAX_IMAGES_PER_SECTION;

  const saveEdit = () => {
    if (!eName.trim()) return;

    onUpdate({ name: eName.trim() });
  ...
  return (
    ...
    >
      ...
      >
        <GripVertical size={14} style={{ color: tk.dimmed }} />
        ...
        {!editing && (
          <div className="flex items-center gap-1">
            <IconBtn icon={Pencil} onClick={() => setEditing(true)} tk={tk} />

            <IconBtn icon={Trash2} onClick={onRemove} tk={tk} danger />
              ...
              {imageCount > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {section.images.map((img, idx) => {
                    const src = getSectionImageUrl(img);
...
function IconBtn({ icon: Icon, onClick, tk, danger = false }) {
  return (
    <button
      onClick={onClick}
      className="w-7 h-7 rounded-lg flex items-center justify-center transition-all cursor-pointer"
      style={{
        color: danger ? "rgba(239,68,68,0.50)" : tk.dimmed,
