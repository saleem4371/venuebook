...
//
// NOTE: the brand reel is ALSO per-category now (see `reel_video` below) —
// it lives on this same row, exactly like `bannerImage` / `video_url`, so it
// survives a refresh instead of only existing as an in-memory File/blob.
...
function parseRow(cat, row) {
  ...
  return {
    ...
    // video (category showcase video)
    youtubeUrl: row.youtube_url || "",
    video_url: row.video_url || "",
    videoFile: null,

    // brand reel — per-category, persisted on this row. Accepts either
    // `brand_reel_url` or the legacy `reel_video` key from the backend so
    // this keeps working regardless of which one the API returns.
    reel_video: row.brand_reel_url || row.reel_video || "",
    reelFile: null,
...
export default function ParentPageManagement() {
  ...
  // parseRow() for the shape. The brand reel (`reelFile` / `reel_video`)
  // lives inside this record now, same as the banner image and showcase
  // video, so it is category-scoped and survives refresh once saved.
  ...
  // ── BRAND REEL (per-category — see catData[cat].reelFile / .reel_video) ────
  ...
  const handleVideoUpload = (e) => {
    ...
    // NOTE: this now matches the "Max 3 MB" copy shown in the requirements
    // card below — previously the check allowed 10 MB while the UI said 3 MB.
    ...
    video.onloadedmetadata = () => {
      ...
      // Store on the active category's record, exactly like bannerImage/videoFile,
      // so it flows through the normal dirty-tracking + save pipeline.
      updCat(activeCat, "reelFile", file);
    };
    video.onerror = () => {
      URL.revokeObjectURL(objectUrl);
  ...
  // SaveParent call keyed by ITS OWN parent_venue_id. After a successful
  // save we adopt whatever reel URL the backend hands back and drop the
  // local File object — that's what makes the reel survive a refresh.
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([...dirtyCats].map(async (cat) => {
        ...
        if (data.reelFile && typeof data.reelFile !== "string") {
          formData.append("brand_reel", data.reelFile);
        }

        const res = await SaveParent(data.parent_venue_id, formData);

        // Adopt the persisted reel URL from the response (fall back to
        // whatever we already had) and clear the local File so the next
        // render reads from the real, saved value instead of a blob.
        // TODO: confirm the exact response key your API returns the
        // uploaded reel under and adjust the fallbacks below if needed.
        const savedReelUrl =
          res?.data?.brand_reel_url ??
          res?.data?.reel_video ??
          res?.data?.result?.brand_reel_url ??
          data.reel_video;

        const savedBannerImage =
          res?.data?.banner_image ?? data.bannerImage;
        const savedVideoUrl =
          res?.data?.video_url ?? data.video_url;
        ...
        setCatData(p => ({
          ...
          [cat]: {
            ...p[cat],
            reelFile: null,
            reel_video: savedReelUrl,
            bannerImage: typeof p[cat]?.bannerImage === "string" ? p[cat].bannerImage : savedBannerImage,
            videoFile: null,
  ...
  // Brand reel preview source — mirrors bannerSrc/videoObjectUrl exactly:
  // a fresh local File wins (instant preview on selection), otherwise fall
  // back to the persisted URL from the row so it shows up after a refresh.
  const reelSrc = useMemo(() => {
    if (acd.reelFile && typeof acd.reelFile !== "string") return URL.createObjectURL(acd.reelFile);
    if (acd.reel_video) {
      return acd.reel_video.startsWith("http")
        ? acd.reel_video
        : `${process.env.NEXT_PUBLIC_AWS_BUCKET_URL}/${acd.reel_video}`;
    }
    return null;
  }, [acd.reelFile, acd.reel_video]);

  useEffect(() => () => {
    if (reelSrc && acd.reelFile && typeof acd.reelFile !== "string") URL.revokeObjectURL(reelSrc);
  }, [reelSrc, acd.reelFile]);

  const removeReel = () => {
    updCat(activeCat, "reelFile", null);
    updCat(activeCat, "reel_video", "");
    setVideoError("");
  };
  ...
  return (
    <div className="relative pb-44 min-h-screen">
      ...
      <div className="space-y-6">
        ...
        {/* ═══ BRAND REEL — per-category (acd.reelFile / acd.reel_video), persists across refresh ═══ */}
        <SectionCard>
          <SectionHeader icon={Film} iconBg="bg-[#a44bf3]/10" iconColor="text-[#a44bf3]" title="Brand Reel"
            subtitle={`Upload a short promotional reel for ${CAT_LABELS[activeCat] ?? activeCat}.`} />
          <div className="space-y-4">
            {videoError && (
            ...
            <label className="group relative flex h-[220px] w-full max-w-[500px] cursor-pointer items-center justify-center overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-slate-50 transition-all hover:border-[#a44bf3] hover:bg-[#faf7ff] dark:border-white/[0.10] dark:bg-white/[0.02] dark:hover:bg-[#a44bf3]/[0.04]">
              ...
              {reelSrc ? (
                <div className="relative h-full w-full">
                  ...
                  <video
                    key={typeof acd.reelFile === "string" ? acd.reelFile : (acd.reelFile?.name ?? acd.reel_video)}
                    controls
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/70 to-transparent p-4">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        <span className="rounded-full bg-white/20 px-2.5 py-1 text-[11px] text-white">
                          {(acd.reelFile.size / 1024 / 1024).toFixed(2)} MB
                        </span>

                        <span className="rounded-full bg-white/20 px-2.5 py-1 text-[11px] text-white">
                          {acd.reelFile.type.replace("video/", "").toUpperCase()}
                        </span>
                  ...
                  {!acd.reelFile && acd.reel_video && (
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/70 to-transparent p-4">
                      <span className="rounded-full bg-emerald-500/80 px-2.5 py-1 text-[11px] text-white font-semibold">
                        Saved
                      </span>
                    </div>
                  )}
                ...
                <div className="px-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    Showcase your {CAT_LABELS[activeCat]?.toLowerCase() ?? "venue"} with a short promotional reel.
                  </p>
