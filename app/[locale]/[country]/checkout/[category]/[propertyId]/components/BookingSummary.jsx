...
/* ─── Line item ──────────────────────────────────────────────────────── */
function LineItem({ label, value, isTotal, isDiscount, tooltip, baseamount = 0, night = 0 }) {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-2 ${
        isTotal
          ? "pt-3 mt-3 border-t border-gray-200 dark:border-neutral-700"
          : ""
      }`}
    >
...
function CategorySummaryRow({ label, value }) {
  ...
  return (
    <div className="flex justify-between text-sm py-1">
      ...
      <span className="font-medium text-gray-700 dark:text-neutral-300 text-end max-w-[60%]">
        {value}
      </span>
    </div>
  );
...
function CategorySnapshot({ normCat, bookingDetails, booking, t }) {
    ...
    case "venues":
      return (
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">
          <CategorySummaryRow label={t("event_type")} value={booking.eventType} />
          <CategorySummaryRow label={t("event_date")} value={booking.date} />
          <CategorySummaryRow
            label={t("shift")}
          ...
          <CategorySummaryRow
            label={t("guest_count")}
            value={booking.guests ? `${booking.guests} guests` : null}
          />
        </div>
      );
    case "farmstays":
      return (
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">
          <CategorySummaryRow label={t("check_in")} value={booking.checkIn} />
          <CategorySummaryRow label={t("check_out")} value={booking.checkOut} />
          <CategorySummaryRow
            label={t("guest_count")}
            value={
              booking.adults
    ...
    case "workspaces":
      return (
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">
          ...
          <CategorySummaryRow
            label={t("seats")}
            value={bookingDetails.seats ? `${bookingDetails.seats} seats` : null}
          />
        </div>
      );
    case "rentals":
      return (
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">
          <CategorySummaryRow
            ...
            value={bookingDetails.rentalStart ? "2 days" : null}
          />
      ...
      return (
        <div className="divide-y divide-gray-100 dark:divide-neutral-800">
          ...
          <CategorySummaryRow
            ...
            value={bookingDetails.participants ? `${bookingDetails.participants} participants` : null}
          />
        </div>
      );
    default:
...
function TrustBlock({ tTrust }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {items.map(({ key, icon }) => (
        <div key={key} className="flex items-center gap-2 text-xs text-gray-500 dark:text-neutral-400">
...
export default function BookingSummary({
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden shadow-sm">
      {/* ── Property card ────────────────────────────────────────────── */}
      <div className="relative h-40 sm:h-48 overflow-hidden bg-gray-100 dark:bg-neutral-800">
        {images?.[0] && (
          <img
            src={images[0]}
            alt={venueData?.child_venue_name || venueData?.venue_name || property?.name || "Property"}
            className="w-full h-full object-cover"
          />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <p className="text-xs text-white/70 font-medium">{venueData?.venue_name}</p>
      ...
      <div className="p-5 space-y-5">
        ...
        <div>
          ...
          <CategorySnapshot normCat={normCat} bookingDetails={bookingDetails} booking={booking} t={t} />
        </div>
        ...
        <div className="space-y-2">
          <LineItem
            label={t("base_price")}
            value={format(summary.baseAmount)}
            night={summary.nights}
            baseamount={venueData?.nightly_amount || venueData?.minPrice}
          />

          {summary.weeklyAmount > 0 && (
            <LineItem
              label="Weekend Charges"
              value={format(summary.weeklyAmount)}
              night={summary.weekends}
              baseamount={venueData?.weekly_amount}
            />
          )}

          {summary.cleaningAmount > 0 && (
            <LineItem label="Cleaning Fee" value={format(summary.cleaningAmount)} />
          )}

          {summary.addonAmount > 0 && (
            <LineItem label={t("addons")} value={format(summary.addonAmount)} />
          )}

          {summary.convenienceFee > 0 && (
            <LineItem label={t("platform_fee")} value={format(summary.convenienceFee)} />
          )}

          <LineItem label={`GST (${summary.gstPercent}%)`} value={format(summary.gstAmount)} />

          {summary.securityDeposit > 0 && (
            <LineItem
              label={t("security_deposit")}
              value={format(summary.securityDeposit)}
          ...
          <LineItem label={t("total")} value={format(summary.grandTotal)} isTotal />
          ...
          {paymentMethod === "cards" && (
            <div className="space-y-2">
              <input
                ...
                placeholder={tPayment("card_number")}
                className={inputClass}
              />
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  placeholder={tPayment("card_expiry")}
                  className={inputClass}
                />
                <input
                  type="password"
              ...
              <input
                ...
                placeholder={tPayment("card_name")}
                className={inputClass}
              />
            </div>
          )}
          ...
          {(paymentMethod === "netbanking" ||
            ...
            <div className="flex items-center justify-center py-6 rounded-xl bg-gray-50 dark:bg-neutral-800 border border-dashed border-gray-200 dark:border-neutral-700">
              <p className="text-sm text-gray-400 dark:text-neutral-500">
                {paymentMethod === "netbanking"
                  ? "Select your bank after proceeding"
                  : paymentMethod === "wallets"
                    ? "PhonePe, Google Pay, Paytm & more"
                    : "Select EMI plan after proceeding"}
              </p>
            </div>
          )}
        </div>
        ...
        >
          {isProcessing ? (
            ...
            <span className="flex items-center justify-center gap-2">
              {tCta(ctaKey)}
              <span className="font-normal opacity-80">·{format(summary.grandTotal)}</span>
            </span>
          )}
        </button>
      </div>
